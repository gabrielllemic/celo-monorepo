...
export const createReport = (oldArtifactsFolder: string, newArtifactsFolder: string, logFunction: (msg: string) => void): ASTBackwardReport => {
  ...
  report.storage = report.storageReports.filter(r => !r.compatible)

  const byChangeType = createIndexByChangeType(report.codeReport.changes, new CategorizerChangeVisitor())
